...
`perl ~user101/software/gene_annot_lzu_v2/utils/split_fasta.pl $genome $name/split_contig no` if(! -e "$name/split_contig");
...
print O "cd $out_dir_path; /data/00/user/user101/software/LTR_FINDER_parallel/LTR_FINDER_parallel -seq $genome_path -threads $cpu -harvest_out -size 5000000 -time 3000\n";
close O;
...
print O "cd $out_dir_path; perl /data/00/user/user101/software/gene_annot_lzu_v2/utils/SplitRunLTRharvest.pl $genome_path /data/00/user/user105/software/LTR_harvest/gt-1.5.10-Linux_x86_64-64bit-complete/bin/gt $name $cpu";
...
cd $id
ln -s $contig $id.fna
/data/00/user/user101/software/LTR_retriever/LTR_retriever/LTR_retriever -genome $id.fna -inharvest $out_dir_path/02.LTRHarvest/$id/$id.harvest.scn -infinder $out_dir_path/01.LTRfinder/$name.genome.fna.finder.combine.scn -threads $cpu
EOF
    }
}else{ # no split
    my $id = $name;
    ...
    /data/00/user/user101/software/LTR_retriever/LTR_retriever/LTR_retriever -genome $name.genome.fna -inharvest $out_dir_path/03.LTR_retriever/$id.merged.scn -threads $cpu\n";
}
close O;
